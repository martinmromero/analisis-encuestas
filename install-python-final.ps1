# Script para configurar Python - Version compatible con PowerShell

Write-Host "Configuracion de Python para Analisis de Sentimientos" -ForegroundColor Cyan
Write-Host "==========================================================="
Write-Host ""
...
function Test-RealPython {
    try {
        ...
        if ($output -notmatch "was not found" -and $output -match "Python") {
            return $true
        }
    } catch {
        return $false
...
if (Test-RealPython) {
    ...
    Write-Host "  - VS Code puede mostrar Python por su extension integrada"
...
if (-not $pythonReady) {
    Write-Host "Opciones de instalacion de Python:" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Opcion 1 - Winget (Recomendado):"
    Write-Host "winget install Python.Python.3.12" -ForegroundColor Green
    Write-Host ""
    Write-Host "Opcion 2 - Manual:"
    Write-Host "Descargar desde https://python.org" -ForegroundColor Green
    Write-Host ""
    
    $choice = Read-Host "Quieres que instale Python automaticamente con winget? (y/n)"
    ...
    if ($choice -eq "y" -or $choice -eq "Y") {
        ...
        try {
            ...
            Write-Host ""
            Write-Host "IMPORTANTE: Debes reiniciar PowerShell para que Python este disponible" -ForegroundColor Yellow
        ...
        Write-Host ""
        Write-Host "Sin Python, solo estaran disponibles los motores de JavaScript:" -ForegroundColor Blue
        Write-Host "   - Natural.js Enhanced (recomendado para español)"
        Write-Host "   - ML-Sentiment"
        Write-Host "   - VADER JavaScript"
        Write-Host ""
        Write-Host "Puedes usar la aplicacion normalmente con estos 3 motores"
...
# Si llegamos aqui, Python esta disponible
...
try {
    ...
    Write-Host "Advertencia: Error actualizando pip (continuando...)" -ForegroundColor Yellow
}

# Paquetes necesarios
$packages = @("textblob", "vaderSentiment", "spacy", "spacytextblob")
...
$engines = @("textblob", "vaderSentiment", "spacy")
$successCount = 0

foreach ($engine in $engines) {
    try {
        python -c "import $engine" 2>$null
        Write-Host "✓ $engine - OK" -ForegroundColor Green
        $successCount++
    } catch {
        Write-Host "✗ $engine - ERROR" -ForegroundColor Red
    }
}
...
if ($successCount -eq $engines.Count) {
    ...
    Write-Host "INSTALACION PARCIAL ($successCount de $($engines.Count) motores)" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "✓ Los motores de JavaScript funcionaran correctamente"
    Write-Host "✗ Algunos motores de Python podrian fallar"
    Write-Host ""
    Write-Host "Puedes usar la aplicacion normalmente" -ForegroundColor Cyan
}
